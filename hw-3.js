...
if (/iPad|Mac|OS|iPhone|Macintosh|Apple|iPod/.test(userAgent) && !window.MSStream) {
...
let clientDeviceYear = Number(prompt('Введите полный год ОС'));

function getInstallationMessage(clientOS, clientDeviceYear) {
    let currentYear = new Date().getFullYear();

    if (clientDeviceYear < 2015) {
        return clientOS === "iOS" ?
            "Установите облегченную версию приложения для iOS" :
            "Установите облегченную версию для Android";
    }
    if (clientDeviceYear >= 2015) {
        return clientOS === "iOS" ?
            "Установите версию приложения для iOS" :
            "Установите версию для Android";
    }
    return "Год введен некорректно"
}
console.log(getInstallationMessage(clientOS, clientDeviceYear));
